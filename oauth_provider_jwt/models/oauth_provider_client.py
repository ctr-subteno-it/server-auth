...
class OAuthProviderClient(models.Model):
    ...
    def _generate_jwt_payload(self, request):
        ...
        if not request.odoo_user and request.refresh_token:
            # Need to (re)get this from the database
            existing_token = self.env['oauth.provider.token'].search([
                ('client_id', '=', request.client.id),
                ('refresh_token', '=', request.refresh_token),
            ])
            if existing_token:
                # Get the user id and remove the former token (a new one will be issued)
                request.odoo_user = existing_token.user_id
        ...
        if request.client.jwt_scope_id:
            ...
            # bytes types are not serializable
            scope_data = {key: val for key, val in scope_data.items() if type(val) != type(b'')}
