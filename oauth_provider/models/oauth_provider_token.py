...
class OAuthProviderToken(models.Model):
    ...
    token_type = fields.Selection(
        ...
        'available.')
    token_md5 = fields.Char(
        compute='_compute_token_md5', store=True)
    ...
    _sql_constraints = [
        ('token_unique_md5', 'UNIQUE (token_md5, client_id)',
    ...
    @api.multi
    @api.depends('token')
    def _compute_token_md5(self):
        for record in self:
            record.token_md5 = hashlib.md5(record.token.encode()).hexdigest()
